...
const PRIORITIES = [
  { value: 'LOW', label: 'Rendah', color: 'text-slate-700 bg-slate-100 border-slate-200' },
  { value: 'MEDIUM', label: 'Sedang', color: 'text-blue-700 bg-blue-50 border-blue-200' },
  { value: 'HIGH', label: 'Tinggi', color: 'text-amber-700 bg-amber-50 border-amber-200' },
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-400 text-sm sm:text-base max-w-xl mx-auto">
            Sampaikan masukan, kendala, atau keluhan Anda secara cepat tanpa perlu login. Tim pengelola kami siap memberikan tindak lanjut terbaik.
          </p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-300">Tingkat Urgensi</label>
                    <div className="relative">
                      <select
                        value={priority}
                      ...
                      >
                        {PRIORITIES.map((pri) => (
                          <option key={pri.value} value={pri.value} className="bg-slate-900 text-white">
                            {pri.label}
                          </option>
                        ))}
                      </select>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                    ...
                    Lokasi / Nomor Unit / Area <span className="text-slate-500 font-normal">(opsional)</span>
                  </label>
                  <input
                    type="text"
                    value={location}
                    onChange={(e) => setLocation(e.target.value)}
                    placeholder="Contoh: Kolam Renang Utama, Unit A-12, Lobi Depan"
                    className="w-full px-4 py-2.5 bg-slate-900/80 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                  />
                </div>
